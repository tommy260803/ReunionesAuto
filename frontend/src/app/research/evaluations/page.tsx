  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Calidad de resúmenes generados por IA</p>
      </div>
      ...
      {averageScore && (
        <div className="bg-gradient-to-r from-indigo-50 to-blue-50 border border-indigo-200 rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Puntaje Promedio Global</h3>
              <p className="text-gray-600 text-sm">Basado en {evaluations.length} evaluaciones</p>
            </div>
            <div className="text-4xl font-bold text-indigo-600">
              {averageScore} <span className="text-lg text-gray-500">/ 5</span>
            </div>
          </div>
        </div>
      )}
            ...
            <tbody className="divide-y divide-gray-200">
              {evaluations.map((ev) => (
                <tr key={ev.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-4 text-gray-500">{ev.omisiones}</td>
                  <td className="px-4 py-4 text-gray-500">{ev.afirmaciones_no_respaldadas}</td>
                  <td className="px-4 py-4 text-gray-500">{ev.contradicciones}</td>
